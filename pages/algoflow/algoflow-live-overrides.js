(function () {
  ...
  function nftPanelHtml() {
    return [
      ...
      "      <p class=\"af-nft-help\">This creates an Algorand Standard Asset with total 1 and decimals 0. Upload media, pin it to IPFS, pin ARC-3 metadata, then sign the ASA create transaction in your wallet.</p>",
      ...
      "          <div class=\"af-nft-field\"><label for=\"nft-upload-broker-url\">Upload broker URL</label><input class=\"af-nft-input\" id=\"nft-upload-broker-url\" name=\"uploadBrokerUrl\" value=\"/api/nft\" placeholder=\"/api/nft\"><span class=\"af-nft-help\">Production path: media and metadata go to the protected same-domain Worker route. No Pinata credential belongs in the website.</span></div>",
      ...
      "          <button class=\"af-nft-btn\" type=\"submit\" data-nft-mint-submit>Mint ARC-3 NFT on TestNet</button>",
      "        </div>",
      "      </form>",
      "      <div class=\"af-nft-status-box\" data-nft-mint-status>Ready. Connect a TestNet wallet and provide pinned ARC-3 metadata.</div>",
      "    </div>",
  ...
  function showNftPanel() {
    ...
    if (panel.getAttribute("data-nft-rendered") !== "nft-studio-v2") {
      panel.innerHTML = nftPanelHtml();
      panel.setAttribute("data-nft-rendered", "nft-studio-v2");
    }
  ...
  async function buildMetadataJson() {
    try {
      ...
      setMintStatus("Metadata JSON built. Download it, pin it, then paste the final metadata URL ending in #arc3.", "success");
    } catch (error) {
  ...
  function wireNftMinting() {
    ...
    getSelectedWallet().catch(function () {});
  }
